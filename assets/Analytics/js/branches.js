...
window.academyOrganizationTabHandlers = {
    dashboardBranchTabs:'filterDashboardByBranch',reportsBranchTabs:'filterReportsByBranch',messagesBranchTabs:'filterMessagesByBranch',notificationsBranchTabs:'filterNotificationsByBranch',studentBranchTabs:'filterStudentsByBranch',staffBranchTabs:'filterStaffByBranch',coursesBranchTabs:'filterCoursesByBranch',financeBranchTabs:'filterFinanceByBranch',rulesBranchTabs:'filterRulesByBranch',schedulesBranchTabs:'filterSchedulesByBranch',instrumentsBranchTabs:'filterInstrumentsByBranch',lessonsBranchTabs:'filterLessonsByBranch',memberSchedulesBranchTabs:'filterMemberSchedulesByBranch',branchSchedulesBranchTabs:'filterBranchSchedulesByBranch',holidayLeavesBranchTabs:'filterHolidayLeavesByBranch',pointsBranchTabs:'filterPointsByBranch',termsBranchTabs:'filterTermsByBranch',approvalsBranchTabs:'filterApprovalsByBranch',badgesBranchTabs:'filterBadgesByBranch',eventsBranchTabs:'filterEventsByBranch',educationsBranchTabs:'filterEducationsByBranch',favoritesBranchTabs:'filterFavoritesByBranch',profilesBranchTabs:'filterProfilesByBranch',publicationsBranchTabs:'filterPublicationsByBranch',ratingsBranchTabs:'filterRatingsByBranch',ratingSummariesBranchTabs:'filterRatingSummariesByBranch',pollsBranchTabs:'filterPollsByBranch',awardsBranchTabs:'filterAwardsByBranch',certificatesBranchTabs:'filterCertificatesByBranch',experiencesBranchTabs:'filterExperiencesByBranch',contractsBranchTabs:'filterContractsByBranch'
};
window.applyAcademyOrganizationTabs = function () {
    if (!document.getElementById('organization-filter-styles')) {
        const style=document.createElement('style');
        style.id='organization-filter-styles';
        style.textContent='.organization-filter-tabs button:not(.bg-indigo-600){background-color:#fff!important;color:#374151!important;border-color:#e5e7eb!important}.organization-filter-tabs button:not(.bg-indigo-600):hover{background-color:#f9fafb!important}.organization-filter-tabs button.bg-indigo-600{background-color:#4f46e5!important;color:#fff!important;border-color:#4f46e5!important}.organization-filter-tabs button.bg-indigo-600:hover{background-color:#4338ca!important;border-color:#4338ca!important}';
        document.head.appendChild(style);
    }
    Object.entries(window.academyOrganizationTabHandlers).forEach(function ([id,handlerName]) {
        const container=document.getElementById(id);if(!container)return;
        container.classList.add('organization-filter-tabs');
        if(window.branchAccount||window.branchSiteAdmin)return;
        window.academyObservedTabs??=new WeakSet();
        if(!window.academyObservedTabs.has(container)){window.academyObservedTabs.add(container);const observer=new MutationObserver(function(){if(container.dataset.organizationTabsPending==='1')return;container.dataset.organizationTabsPending='1';requestAnimationFrame(function(){delete container.dataset.organizationTabsPending;window.applyAcademyOrganizationTabs();});});observer.observe(container,{childList:true,subtree:true});}
        const first=container.querySelector('button');if(!first)return;
        if(first.textContent.trim()!=='همه')first.textContent='همه';
        if(first.dataset.value!=='all')first.dataset.value='all';
        if(container.querySelector('[data-value="academy"],[data-staff-organization="academy"]'))return;
        const button=first.cloneNode(true);button.textContent='آموزشگاه';button.dataset.value='academy';button.dataset.staffOrganization='academy';const academyActive=container.dataset.selectedValue==='academy';button.classList.toggle('bg-indigo-600',academyActive);button.classList.toggle('text-white',academyActive);button.classList.toggle('border-indigo-600',academyActive);button.classList.toggle('border-gray-200',!academyActive);button.onclick=function(){container.dataset.selectedValue='academy';const handler=window[handlerName];if(typeof handler==='function')handler('academy');setTimeout(function(){container.querySelectorAll('button').forEach(b=>{const active=b.dataset.value==='academy'||b.dataset.staffOrganization==='academy';b.classList.toggle('bg-indigo-600',active);b.classList.toggle('text-white',active);b.classList.toggle('border-indigo-600',active);b.classList.toggle('border-gray-200',!active);});},0);};first.insertAdjacentElement('afterend',button);
    });
};
...
let branchesViewInitialized = false;
let branchSortField = '';
...
let branchRealtimeVersion = null;
let branchRealtimeBusy = false;
const branchRealtimeChannel = 'BroadcastChannel' in window ? new BroadcastChannel('sornaz-admin-data') : null;
...
window.setBranchesView=function(view){branchesView=view;document.getElementById('branchesCards')?.classList.toggle('hidden',view!=='cards');document.getElementById('branchesTableWrap')?.classList.toggle('hidden',view!=='table');[['branchesTableViewButton','table'],['branchesCardViewButton','cards']].forEach(([id,mode])=>{const button=document.getElementById(id),active=view===mode;if(!button)return;button.classList.toggle('border-indigo-600',active);button.classList.toggle('bg-indigo-600',active);button.classList.toggle('text-white',active);button.classList.toggle('border-gray-200',!active);button.classList.toggle('bg-white',!active);button.classList.toggle('text-gray-700',!active);button.setAttribute('aria-pressed',active?'true':'false');});};
async function loadBranches(){try{const data=await branchRequest('/academy/admin/branches',null,'GET');branchCsrfToken=data.csrf_token||branchCsrfToken;allBranches=data.branches||[];filteredBranches=[...allBranches];allBranchTypes=data.types||[];allBranchAcademies=data.academies||[];window.branchReadOnly=Boolean(data.read_only);window.branchSiteAdmin=Boolean(data.site_admin);window.branchAccount=Boolean(data.branch_account);window.branchCanCreate=data.can_create_branch!==false;window.branchCanDelete=data.can_delete_branch!==false;window.applyFixedBranchScope();window.applyAcademyOrganizationTabs();const addButton=document.getElementById('addBranchButton');if(addButton)addButton.classList.toggle('hidden',!window.branchCanCreate);iranProvinces=(data.provinces||[]).map(item=>({id:item.province_id,name:item.province_name}));iranCounties=(data.counties||[]).map(item=>({id:item.county_id,province_id:item.province_id,name:item.county_name}));const description=document.getElementById('branchesScopeDescription');if(description)description.textContent=window.branchSiteAdmin?'مدیریت سراسری تمام آموزشگاه‌ها و شعب ثبت‌شده':(window.branchAccount?'اطلاعات و مدیریت همین شعبه':'مدیریت شعب آموزشگاه');document.getElementById('branchesAcademiesCount').textContent=localizeBranchNumber(allBranchAcademies.length||new Set(allBranches.map(item=>item.academy_id)).size);window.renderBranchFilters();window.filterBranches();if(!branchesViewInitialized){branchesView=window.branchSiteAdmin?'table':'cards';branchesViewInitialized=true;}window.setBranchesView(branchesView);window.dispatchEvent(new CustomEvent('academy-data-loaded',{detail:data}));window.applyFixedBranchScope();window.applyAcademyOrganizationTabs();}catch(error){alert(error.message);}}

async function pollBranchesRealtime(){if(branchRealtimeBusy||document.hidden||!document.querySelector('[data-branches-root]'))return;branchRealtimeBusy=true;try{const state=await branchRequest('/academy/admin/branches/realtime-version',null,'GET');if(branchRealtimeVersion===null){branchRealtimeVersion=state.version;return;}if(state.version!==branchRealtimeVersion){branchRealtimeVersion=state.version;editingBranchRowId=null;await loadBranches();branchRealtimeChannel?.postMessage({resource:'branches',version:state.version});}}catch(error){}finally{branchRealtimeBusy=false;}}
branchRealtimeChannel?.addEventListener('message',async event=>{if(event.data?.resource!=='branches'||event.data.version===branchRealtimeVersion)return;branchRealtimeVersion=event.data.version;editingBranchRowId=null;await loadBranches();});
...
window.cycleBranchStatus=async function(id,button=null){if(button?.disabled)return;if(button)button.disabled=true;try{const updated=await branchRequest(`/academy/admin/branches/${id}/status`,{});const index=allBranches.findIndex(item=>item.id===id);if(index>=0)allBranches[index]=updated;window.filterBranches();branchRealtimeChannel?.postMessage({resource:'branches',version:`local-${Date.now()}`});}catch(error){alert(error.message);}finally{if(button?.isConnected)button.disabled=false;}};
window.handleAdminBranchCreated=function(branch){if(!branch)return;const index=allBranches.findIndex(item=>item.id===branch.id);if(index>=0)allBranches[index]=branch;else allBranches.unshift(branch);branchesCurrentPage=1;window.filterBranches();branchRealtimeChannel?.postMessage({resource:'branches',version:`local-${Date.now()}`});};
