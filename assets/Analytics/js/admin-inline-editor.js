(function () {
    ...
    function mark(root=document) {
        ...
        nodes.forEach(node=>{const parent=node.parentElement;if(parent.dataset.inlineTranslationKey)return;const current=normalize(node.nodeValue);const prefix=location.pathname.startsWith('/analytics/admin-panel')?'admin':'site';const generatedKey=`${prefix}.inline.${hash(current)}`;const row=byText.get(current)||byKey.get(generatedKey);const key=row?.key||generatedKey;const marker=document.createElement('span');marker.dataset.inlineTranslationKey=key;marker.dataset.inlineOriginalText=current;if(row){marker.dataset.inlineKnown='1';marker.textContent=document.documentElement.lang==='en'?row.en:row.fa;}else marker.textContent=node.nodeValue;node.replaceWith(marker);});
