(function () {
    ...
    function mark(root=document) {
        ...
        nodes.forEach(node=>{const parent=node.parentElement;if(parent.dataset.inlineTranslationKey)return;const current=normalize(node.nodeValue);const row=byText.get(current);const key=row?.key || `admin.inline.${hash(current)}`;const marker=document.createElement('span');marker.dataset.inlineTranslationKey=key;marker.dataset.inlineOriginalText=current;if(row)marker.dataset.inlineKnown='1';marker.textContent=node.nodeValue;node.replaceWith(marker);});
        document.querySelectorAll('input[placeholder],textarea[placeholder]').forEach(el=>{if(el.closest('[data-no-inline-edit],#modalContainer'))return;const current=normalize(el.placeholder);if(!current)return;const row=byText.get(current);el.dataset.inlineTranslationKey=row?.key||`admin.inline.${hash(current)}`;el.dataset.inlineOriginalText=current;el.dataset.inlineAttribute='placeholder';if(row)el.dataset.inlineKnown='1';});
    }
    ...
    document.addEventListener('DOMContentLoaded',()=>{setMode(false);mark();new MutationObserver(records=>{if(!editMode)return;records.forEach(record=>record.addedNodes.forEach(node=>{if(node.nodeType===1)mark(node);}));}).observe(document.getElementById('mainContent')||document.body,{childList:true,subtree:true});});
    window.AdminInlineEditor={setMode,save,close};
})();
