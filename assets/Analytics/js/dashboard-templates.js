(function () {
    ...
    window.getDashboardActionItemsHTML=function(list){
        ...
        return list.map(function(item){
            if(item.type==='user_merge_pending')return `<article class="rounded-2xl border border-indigo-200 bg-indigo-50 p-4"><div class="font-bold text-indigo-800">درخواست ترکیب حساب‌های کاربری #${escapeHtml(item.requestId)}</div><p class="mt-2 text-sm text-gray-700">انتقال سوابق کاربر #${escapeHtml(item.sourceUserId)} با شماره عضویت #${escapeHtml(item.memberId)} به ${escapeHtml(item.targetName)} (#${escapeHtml(item.targetUserId)})</p><p class="mt-1 text-xs text-gray-500">${escapeHtml(item.reason)} · ${escapeHtml(item.date||'')}</p><div class="mt-4 flex gap-2"><button onclick="decideDashboardUserMerge(${item.requestId},'approved')" class="rounded-xl bg-emerald-600 px-4 py-2 text-sm text-white">تأیید و انتقال</button><button onclick="decideDashboardUserMerge(${item.requestId},'rejected')" class="rounded-xl border border-red-300 px-4 py-2 text-sm text-red-700">رد درخواست</button></div></article>`;
