...
window.payFinanceInstallment = async function (invoiceId, installmentId, isInline) {
    ...
    if (!confirmed) return;
    try {
        const payment = await financeApi('/academy/admin/term-invoices/' + invoiceId + '/installments/' + installmentId + '/zarinpal', {});
        if (!payment.redirectUrl) throw new Error('لینک انتقال به زرین‌پال دریافت نشد.');
...
(function initFinance() {
    setTimeout(async function () {
        if (document.getElementById('financeTable')) {
            showFinancePaymentCallback();
            loadAcademySubscriptions();
            try {
                await loadFinanceInvoices();
            } catch (error) {
                console.error(error);
