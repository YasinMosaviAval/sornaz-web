...
const readTermFormWithoutTimezones=readTermForm;readTermForm=function(prefix){const data=readTermFormWithoutTimezones(prefix),box=termField(prefix,'SessionsContainer');if(box){const zones=[...box.querySelectorAll('.term-session-timezone')];data.sessions.forEach((session,index)=>session.timezoneId=Number(zones[index]?.value||0));}return data;};
...
window.syncTermSessionTimes=function(prefix){const box=termField(prefix,'SessionsContainer');if(!box)return;const starts=[...box.querySelectorAll('.term-session-start')],durations=[...box.querySelectorAll('.term-session-duration')],first=starts[0]?.value||'';starts.slice(1).forEach(x=>{x.dataset.preferredStart=first;if([...x.options].some(o=>o.value===first))x.value=first;});durations.slice(1).forEach(x=>x.value=durations[0]?.value||'');Promise.all(starts.slice(1).map((_,i)=>refreshTermSessionAvailability(prefix,i+1)));};
...
window.refreshTermSessionDates=function(prefix,force=false){const repeat=termField(prefix,'RepeatType')?.value||'no-period',box=termField(prefix,'SessionsContainer');if(!box)return;const inputs=[...box.querySelectorAll('.term-session-date')],activeIndex=inputs.indexOf(document.activeElement),first=inputs[0]?.value;if(!force&&activeIndex>0){inputs[activeIndex].dataset.manual='1';return;}if(repeat==='no-period'||!first)return;const days={'week':7,'2-week':14,'3-week':21,'4-week':28},start=new Date(first+'T12:00:00');inputs.forEach((input,i)=>{if(!i||(!force&&input.dataset.manual==='1'))return;const date=new Date(start);if(days[repeat])date.setDate(date.getDate()+days[repeat]*i);else if(repeat==='month')date.setMonth(date.getMonth()+i);else if(repeat==='year')date.setFullYear(date.getFullYear()+i);input.value=date.toISOString().slice(0,10);input.dataset.manual='0';});};
window.termSessionDateChanged=function(prefix,index,input){if(index===0)refreshTermSessionDates(prefix);else input.dataset.manual='1';refreshTermSessionAvailability(prefix,index);};
