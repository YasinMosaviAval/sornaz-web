...
async function resolveAutomaticTermSessionDates(prefix){
    ...
    for(let index=1;index<inputs.length;index++){
        ...
        const label=organization?.kind==='academy'?'آموزشگاه':'شعبه';
        const skipped=rejected.map(date=>window.localizedDateValue?.(date)||date).join('، '),replacement=window.localizedDateValue?.(candidate)||candidate;
        setTermSessionNote(box,index,rejected.length?`${label} در تاریخ ${skipped} تعطیل است؛ تاریخ جلسه به ${replacement} منتقل شد.`:'');
