(function () {
    ...
    window.getInstrumentRowHTML = function (item, levelTitle) {
        ...
        return `
            ...
            <td class="py-4 px-5 text-left">
                <div class="inline-flex flex-nowrap items-center gap-2 whitespace-nowrap">
                    <button onclick="viewInstrument(${item.id})" class="text-indigo-600 hover:underline text-sm">جزئیات</button>
                    <button onclick="toggleInstrumentInlineEdit(${item.id})" class="text-gray-500 hover:text-indigo-600 text-sm">ویرایش</button>
                    <button onclick="deleteInstrument(${item.id})" class="text-red-500 hover:text-red-700 text-sm">حذف</button>
                </div>
            </td>`;
    };
    window.getInstrumentEmptyRowHTML = function () {
        return `<tr><td colspan="7" class="py-12 text-center text-gray-400">موردی یافت نشد</td></tr>`;
    };
    window.getInstrumentInlineExpandRowHTML = function (item) {
        return `<td colspan="7" class="p-5 border-t">${window.getInstrumentInlineEditRowHTML ? window.getInstrumentInlineEditRowHTML(item) : ''}</td>`;
    };

    function formFields(item, prefix) {
        const id = function (n) { return prefix ? prefix + n : 'inst' + n; };
        const branches = (typeof allBranches !== 'undefined' ? allBranches : []).map(function (b) { return { value: b.id, label: b.name }; });
        const instruments = (typeof window.sampleInstruments !== 'undefined' ? window.sampleInstruments : []).map(function (i) { return { value: i.id, label: i.title }; });
        const levels = (typeof window.instrumentLevels !== 'undefined' ? window.instrumentLevels : []).map(function (l) { return { value: l.level_id, label: l.title + ' (' + l.type + ')' }; });
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                ...
                <div>
                    ...
                    <select id="${id('Select')}" class="w-full border border-gray-300 rounded-2xl py-3.5 px-5">
                        ${renderOptions(instruments, item.instrument_id)}
                    </select>
                    <button type="button" onclick="promptAddInstrumentType()" class="text-sm text-indigo-600 mt-1">+ ساز جدید</button>
                </div>
                <div>
                ...
                <div class="flex items-end">
                    <label class="flex items-center gap-2 text-sm pb-3">
                        ...
                        ساز اصلی (فقط یکی برای هر کاربر)
                    </label>
                </div>
    ...
    window.getInstrumentInlineEditRowHTML = function (item) {
        return `<div class="space-y-6">
            ${formFields(item, 'inlineInst' + item.id)}
            <div class="flex flex-col sm:flex-row gap-4 pt-2">
                <button onclick="saveInlineInstrument(${item.id})" class="w-full sm:w-auto min-w-[140px] bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-4 rounded-2xl font-medium">ذخیره</button>
                <button onclick="toggleInstrumentInlineEdit(${item.id})" class="w-full sm:w-auto min-w-[140px] border border-gray-300 px-5 py-4 rounded-2xl hover:bg-gray-50">انصراف</button>
            </div>
        </div>`;
    };
    window.getInstrumentAddModalHTML = function () {
            ...
            <div class="bg-white rounded-3xl w-full max-w-2xl my-8 shadow-2xl overflow-hidden" onclick="event.stopPropagation()">
                ...
                <div class="p-8 space-y-6 max-h-[75vh] overflow-y-auto">
                    ...
                    <div class="flex gap-4 pt-4">
                        <button onclick="saveInstrument()" class="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-4 rounded-2xl font-medium">ذخیره</button>
    ...
    window.getInstrumentEditModalHTML = function (item) {
        return `<div class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 overflow-y-auto" onclick="if(event.target===this) closeModal()">
            <div class="bg-white rounded-3xl w-full max-w-2xl my-8 shadow-2xl overflow-hidden" onclick="event.stopPropagation()">
                <div class="px-8 py-5 border-b flex justify-between items-center">
                ...
                <div class="p-8 space-y-6 max-h-[75vh] overflow-y-auto">
                    ${formFields(item, 'editInst')}
                    <div class="flex gap-4 pt-4">
                        <button onclick="saveEditedInstrument(${item.id})" class="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-4 rounded-2xl font-medium">ذخیره تغییرات</button>
                        <button onclick="closeModal()" class="flex-1 border border-gray-300 py-4 rounded-2xl hover:bg-gray-50">انصراف</button>
                    </div>
                </div>
            </div>
    ...
    window.getInstrumentDetailsModalHTML = function (item, levelTitle) {
        return `<div class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 overflow-y-auto" onclick="if(event.target===this) closeModal()">
            <div class="bg-white rounded-3xl w-full max-w-2xl my-8 shadow-2xl" onclick="event.stopPropagation()">
                <div class="sticky top-0 bg-white px-8 py-5 border-b flex justify-between items-center rounded-t-3xl">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 mt-1">${escapeHtml(levelTitle || '')} ${item.is_primary ? '— ساز اصلی' : ''}</p>
                    </div>
                    <div class="flex items-center gap-2">
                        <button onclick="editInstrument(${item.id})" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2.5 rounded-xl text-sm">ویرایش</button>
                        <button onclick="closeModal()" class="text-3xl text-gray-300 hover:text-gray-500">×</button>
                    </div>
                </div>
    ...
    window.getInstrumentPDFModalHTML = function (cols) {
        return `<div class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 overflow-y-auto" onclick="if(event.target===this) closeModal()">
            <div class="bg-white rounded-3xl w-full max-w-2xl my-8 shadow-2xl overflow-hidden" onclick="event.stopPropagation()">
                <div class="sticky top-0 bg-white px-8 py-5 border-b flex justify-between items-center rounded-t-3xl">
                    <h2 class="text-2xl font-bold">تنظیمات خروجی PDF سازها</h2>
                ...
                <div class="p-8 space-y-6" style="max-height:calc(100vh - 10rem);overflow-y:auto;">
                    <input id="instPdfTitle" type="text" value="گزارش سازهای آموزشگاه" class="w-full border border-gray-300 rounded-2xl py-3.5 px-5">
                    <input id="instPdfSubtitle" type="text" value="لیست سازها، سطح و وضعیت" class="w-full border border-gray-300 rounded-2xl py-3.5 px-5">
                    <div class="grid grid-cols-2 gap-5">
                        <select id="instPdfFormat" class="w-full border border-gray-300 rounded-2xl py-3.5 px-5"><option value="a4">A4</option><option value="letter">Letter</option></select>
                        <select id="instPdfOrientation" class="w-full border border-gray-300 rounded-2xl py-3.5 px-5"><option value="landscape">افقی</option><option value="portrait">عمودی</option></select>
                    </div>
                    <input id="instPdfFooter" type="text" value="تولید شده توسط سیستم مدیریت آموزشگاه" class="w-full border border-gray-300 rounded-2xl py-3.5 px-5">
                    <div class="grid grid-cols-2 gap-2">
                        ${cols.map(function (c) { return `<label class="inline-flex items-center gap-2 text-sm"><input type="checkbox" id="instPdfCol-${c.field}" checked> ${c.label}</label>`; }).join('')}
                    </div>
                    <div class="grid grid-cols-3 gap-5">
                        <input id="instPdfHeaderColor" type="color" value="#eff6ff" class="w-full h-12 border rounded-2xl p-2">
                        <input id="instPdfEvenRowColor" type="color" value="#ffffff" class="w-full h-12 border rounded-2xl p-2">
                        <input id="instPdfOddRowColor" type="color" value="#f8fafc" class="w-full h-12 border rounded-2xl p-2">
                    </div>
                    <label class="flex items-center gap-3"><input id="instPdfIncludeDate" type="checkbox" checked> <span class="text-sm">نمایش تاریخ استخراج</span></label>
                    <div class="flex gap-4 pt-4">
                        <button onclick="generateInstrumentsPDF()" class="flex-1 bg-indigo-600 text-white py-4 rounded-2xl">ایجاد PDF</button>
    ...
    window.getInstrumentPDFPageHTML = function (pageNumber, rows, isFirstPage, options) {
        const o = options;
