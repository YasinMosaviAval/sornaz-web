(function () {
    ...
    window.getBranchTableRowHTML = function (branch) {
        return `<tr class="border-t border-gray-100"><td class="p-4">${branch.academy_name || '—'}</td><td class="p-4 font-medium">${branch.name}${branch.is_main ? ' <span class="text-xs text-amber-700">(اصلی)</span>' : ''}</td><td class="p-4">${branch.type}</td><td class="p-4">${window.getBranchPhysicalTypeLabel(branch.physical_type)}</td><td class="p-4">${branch.manager || '—'}</td><td class="p-4">${branch.status}</td><td class="p-4"><div class="flex gap-2"><button onclick="viewBranch(${branch.id})" class="rounded-lg border px-3 py-1.5 text-indigo-600">مشاهده</button><button onclick="editBranch(${branch.id})" class="rounded-lg bg-indigo-600 px-3 py-1.5 text-white">ویرایش</button><button onclick="deleteBranch(${branch.id})" class="rounded-lg bg-red-600 px-3 py-1.5 text-white">حذف</button></div></td></tr>`;
    };

    window.getBranchInlineEditRowHTML = function (branch) { return `<tr class="border-t border-indigo-100 bg-indigo-50/60"><td colspan="7" class="p-5">${branchForm(branch, true, true)}</td></tr>`; };
    ...
    window.getBranchAddressFieldHTML = function (address = {}) {
        ...
        return `<div class="border border-gray-200 rounded-2xl p-4 space-y-3 mb-3 address-block">
            <div class="grid grid-cols-2 gap-3"><div><label class="text-xs text-gray-500 mb-1 block">استان</label><select class="addr-province border border-gray-300 rounded-2xl py-3 px-4 w-full" onchange="updateBranchCountySelect(this)"><option value="">انتخاب استان</option>${getProvinceOptions(selectedProvince)}</select></div><div><label class="text-xs text-gray-500 mb-1 block">شهر</label><select class="addr-city border border-gray-300 rounded-2xl py-3 px-4 w-full disabled:bg-gray-100 disabled:text-gray-400" ${selectedProvince ? '' : 'disabled'}>${selectedProvince ? getCityOptions(selectedCity, selectedProvince) : '<option value="">ابتدا استان را انتخاب کنید</option>'}</select></div></div>
