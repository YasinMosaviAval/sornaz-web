(function () {
    ...
    function statusButton(branch) {
        const colors = branch.status === 'فعال' ? 'bg-green-100 text-green-700 hover:bg-green-200' : (branch.status === 'حذف‌شده' ? 'bg-red-100 text-red-700 hover:bg-red-200' : 'bg-yellow-100 text-yellow-700 hover:bg-yellow-200');
        return `<button type="button" onclick="cycleBranchStatus(${branch.id},this)" class="px-3 py-1 rounded-full text-xs transition ${colors}" title="تغییر وضعیت">${branch.status}</button>`;
    }
