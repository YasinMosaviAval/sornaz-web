...
window.localizedDateValue=jalali;
...
window.syncLocalizedDateInput=function(input){
    if(!input)return;
    const visible=input.closest('.localized-date-field')?.querySelector('input[data-jdp]');
    if(!visible)return;
    if(visible.value!==jalali(input.value))visible.value=jalali(input.value);
    visible.disabled=input.disabled;
};
window.initLocalizedDateInputs=function(root){if(!fa)return;root=root||document;const selector='input[type="date"]:not([data-localized-ready])',inputs=[];if(root.matches?.(selector))inputs.push(root);root.querySelectorAll?.(selector).forEach(input=>inputs.push(input));inputs.forEach(function(input){if(!input.id)input.id='localizedDate'+Math.random().toString(36).slice(2);input.dataset.localizedReady='1';const wrapper=document.createElement('div');wrapper.className='localized-date-field';const visible=document.createElement('input');visible.type='text';visible.value=jalali(input.value);visible.disabled=input.disabled;visible.placeholder='۱۴۰۵/۰۱/۰۱';visible.className=input.className;visible.setAttribute('data-jdp','');visible.setAttribute('data-jdp-init-date',visible.value||'');visible.setAttribute('data-jdp-target-value-input','#'+CSS.escape(input.id));visible.setAttribute('data-jdp-target-value-type','gregorian');visible.setAttribute('autocomplete','off');visible.setAttribute('aria-label',input.getAttribute('aria-label')||input.getAttribute('title')||'انتخاب تاریخ');const button=document.createElement('button');button.type='button';button.className='localized-date-trigger';button.setAttribute('aria-label','باز کردن تقویم');button.setAttribute('title','انتخاب تاریخ');button.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 2v3M17 2v3M3.5 9h17M5 4h14a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2Zm2 9h2v2H7v-2Zm4 0h2v2h-2v-2Zm4 0h2v2h-2v-2Zm-8 4h2v2H7v-2Zm4 0h2v2h-2v-2Z"/></svg>';input.before(wrapper);input.type='hidden';wrapper.append(visible,button,input);button.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();if(visible.disabled)return;visible.focus();openCalendar(visible);});visible.addEventListener('change',()=>{setTimeout(()=>{window.syncLocalizedDateInput(input);input.dispatchEvent(new Event('change',{bubbles:true}));},0);});input.addEventListener('change',()=>window.syncLocalizedDateInput(input));});};
