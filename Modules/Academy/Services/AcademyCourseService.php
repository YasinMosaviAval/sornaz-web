...
class AcademyCourseService
{
    ...
    public function saveCourse(int $actor,array $data,int $id=0): array
    {
        ...
        $values=['branch_id'=>(int)$branch['branch_id'],'lesson_id'=>$lessonId,'level_id'=>$levelId,'teacher_capacity'=>$teacherCapacity,'student_capacity'=>$studentCapacity,'status'=>$statuses[$data['status']??'pending']??'pending','updated_at'=>date('Y-m-d H:i:s'),'updated_by'=>$actor,'deleted_at'=>null,'deleted_by'=>null];
    ...
    {
        $result=$this->seedCoursesRows($actor);
