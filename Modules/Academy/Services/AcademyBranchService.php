...
class AcademyBranchService {
    ...
    public function availabilityExceptions(int $actor, bool $siteAdmin=false): array {
        $catalog=$this->memberSchedules($actor,$siteAdmin);$members=$catalog['members'];$manageableIds=array_map(fn($m)=>(int)$m['id'],$members);$userIds=array_values(array_unique(array_map(fn($m)=>(int)$m['user_id'],$members)));
        ...
        foreach($rows as$r){$id=(int)$r['user_availability_id'];$mid=(int)($r['member_id']??0);$m=$membersById[$mid]??($membersByUser[(int)$r['user_id']][0]??null);if(!$m)continue;$organizationUserId=(int)($m['organizationUserId']??0);$branchId=(int)($m['branch_id']??$m['branchId']??0);$branchName=(string)($m['branchName']??'سازمان دیگر');$start=$r['start_time']?substr((string)$r['start_time'],0,5):'';$end=$r['end_time']?substr((string)$r['end_time'],0,5):'';$slots=[];if($start&&$end)for($n=((int)substr($start,0,2))*60+(int)substr($start,3,2),$until=((int)substr($end,0,2))*60+(int)substr($end,3,2);$n<$until;$n+=30)$slots[]=sprintf('%02d:%02d',intdiv($n,60),$n%60);$items[]=['id'=>$id,'memberId'=>(int)$r['user_id'],'membershipId'=>$mid,'organizationUserId'=>$organizationUserId,'name'=>(string)($m['name']??('کاربر '.$r['user_id'])),'date'=>(string)$r['date'],'slots'=>$slots,'timeLabel'=>$start&&$end?$start.'-'.$end:'تمام‌روز','type'=>(string)$r['unavailable_type'],'typeLabel'=>$typeLabels[$r['unavailable_type']]??$r['unavailable_type'],'timezone'=>$timezones[(int)($r['timezone_id']??0)]??'Asia/Tehran','branchId'=>$branchId,'branchName'=>$branchName,'status'=>$statusLabels[$r['status']]??'در انتظار تأیید','summary'=>$texts[$id]['summary']??'','description'=>$texts[$id]['description']??'','readOnly'=>!in_array($mid,$manageableIds,true)];}
    ...
    public function saveAvailabilityException(int$actor,array$d,int$id=0,bool$siteAdmin=false):array {
        ...
        $catalog=$this->availabilityExceptions($actor,$siteAdmin);$membershipId=(int)($d['membershipId']??0);$member=null;foreach($catalog['members']as$m)if((int)$m['id']===$membershipId)$member=$m;if(!$member)throw new RuntimeException('عضو انتخاب‌شده معتبر نیست.');$date=trim((string)($d['date']??''));if(!preg_match('/^\d{4}-\d{2}-\d{2}$/',$date)||!strtotime($date))throw new RuntimeException('تاریخ معتبر الزامی است.');$type=(string)($d['type']??'');if(!in_array($type,['holiday','closed','unavailable','busy','vacation','blocked'],true))throw new RuntimeException('نوع عدم دسترسی معتبر نیست.');$ranges=array_values((array)($d['ranges']??[]));if(!$ranges)throw new RuntimeException('حداقل یک بازه زمانی الزامی است.');$timezoneName=trim((string)($d['timezone']??'Asia/Tehran'))?:'Asia/Tehran';$timezone=DB::table('f_timezone')->where('timezone',$timezoneName)->whereNull('deleted_at')->first();if(!$timezone)throw new RuntimeException('منطقه زمانی معتبر نیست.');$pending=!$this->staffActorApproves($actor);$now=date('Y-m-d H:i:s');
        return transaction(function()use($actor,$d,$id,$membershipId,$member,$date,$type,$ranges,$timezone,$pending,$now){$saved=[];foreach($ranges as$i=>$range){$start=(string)($range['start']??'');$end=(string)($range['end']??'');if(!preg_match('/^(?:[01]\d|2[0-3]):[0-5]\d$/',$start)||!preg_match('/^(?:[01]\d|2[0-3]):[0-5]\d$/',$end)||$start>=$end)throw new RuntimeException('بازه زمانی معتبر نیست.');$values=['user_id'=>(int)$member['user_id'],'member_id'=>$membershipId,'date'=>$date,'day_of_week'=>null,'start_time'=>$start.':00','end_time'=>$end.':00','timezone_id'=>(int)$timezone['timezone_id'],'status'=>$pending?'pending':'available','unavailable_type'=>$type,'is_repeating'=>0,'repeat_period'=>'none','is_closed'=>0,'priority'=>$i+1,'approved_at'=>$pending?null:$now,'approved_by'=>$pending?null:$actor,'updated_at'=>$now,'updated_by'=>$actor,'deleted_at'=>null,'deleted_by'=>null];if($id&&$i===0){$existing=DB::table('user_availabilities')->where('user_availability_id',$id)->whereNotNull('unavailable_type')->whereNull('deleted_at')->first();if(!$existing)throw new RuntimeException('رکورد موردنظر یافت نشد.');DB::table('user_availabilities')->where('user_availability_id',$id)->update($values);$savedId=$id;}else$savedId=DB::table('user_availabilities')->insertGetId(['created_at'=>$now,'created_by'=>$actor]+$values);$this->setTranslations('user_availabilities',$savedId,['summary'=>trim((string)($d['summary']??'')),'description'=>trim((string)($d['description']??''))],$actor);$saved[]=$savedId;}return['ids'=>$saved];});
    }
