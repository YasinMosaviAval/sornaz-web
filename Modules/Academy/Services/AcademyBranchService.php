...
class AcademyBranchService {
    ...
    public function bootstrap(int $ownerUserId, bool $siteAdmin = false): array {
        ...
        if (!$branchAccount) {
            try {
                ...
                if($managed&&$managed['branch_id']!==null)$branchAccount=DB::table('academy_branches')->where('branch_id',(int)$managed['branch_id'])->whereNull('deleted_at')->first();
            }
        }
        if ($branchAccount) $academy = DB::table('academies')->where('academy_id', (int)$branchAccount['academy_id'])->whereNull('deleted_at')->first();
        if (!$academy) throw new RuntimeException('آموزشگاه مرتبط یافت نشد.');
        $this->normalizeMain((int)$academy['academy_id'], $ownerUserId);
    ...
    public function realtimeVersion(int $ownerUserId, bool $siteAdmin = false): array {
        $branchAccount = null;
        if ($siteAdmin) {
            $branches = $this->allBranches();
        } else {
            ...
            if ($branchAccount) $branches = [$this->decorate($branchAccount)];
            else {
                if (!$academy) throw new RuntimeException('آموزشگاه مرتبط یافت نشد.');
    ...
    public function memberSchedules(int $actor, bool $siteAdmin=false): array {
        $staff=$this->staffData($actor,$siteAdmin);$sourceMembers=$staff['members']??[];$sourceIds=array_values(array_unique(array_map(fn($m)=>(int)$m['id'],$sourceMembers)));$studentMemberships=[];foreach($sourceIds?DB::table('academy_branch_course_term_waiting_list')->whereIn('member_id',$sourceIds)->whereNull('deleted_at')->get():[]as$row)$studentMemberships[(int)$row['member_id']]=true;foreach($sourceIds?DB::table('academy_branch_course_term_enrollments')->whereIn('member_id',$sourceIds)->where('type','student')->whereNull('deleted_at')->get():[]as$row)$studentMemberships[(int)$row['member_id']]=true;$members=array_values(array_filter($sourceMembers,fn($m)=>!isset($studentMemberships[(int)$m['id']])&&!in_array($m['type']??'',['student','waiting','waiting-list'],true)));
        $manageableIds=array_values(array_unique(array_map(fn($m)=>(int)$m['id'],$members)));$userIds=array_values(array_unique(array_map(fn($m)=>(int)$m['user_id'],$members)));
        $allMemberships=$userIds?DB::table('academy_branch_members')->whereIn('user_id',$userIds)->whereNull('deleted_at')->get():[];$membershipMap=[];foreach($allMemberships as$m)$membershipMap[(int)$m['member_id']]=$m;
        $names=[];foreach($userIds?DB::table('translations')->where('table_name','users')->where('field','full_name')->where('locale','fa')->whereIn('table_id',$userIds)->whereNull('deleted_at')->get():[]as$t)$names[(int)$t['table_id']]=$t['value'];
        $branchIds=array_values(array_unique(array_filter(array_map(fn($m)=>(int)($m['branch_id']??0),$allMemberships))));$branchNames=[];foreach($branchIds?DB::table('translations')->where('table_name','academy_branches')->where('field','name')->where('locale','fa')->whereIn('table_id',$branchIds)->whereNull('deleted_at')->get():[]as$t)$branchNames[(int)$t['table_id']]=$t['value'];
        $academyIds=array_values(array_unique(array_map(fn($m)=>(int)$m['academy_id'],$allMemberships)));$academyNames=[];foreach($academyIds?DB::table('translations')->where('table_name','academies')->where('field','title')->where('locale','fa')->whereIn('table_id',$academyIds)->whereNull('deleted_at')->get():[]as$t)$academyNames[(int)$t['table_id']]=$t['value'];
        $rows=$userIds?DB::table('user_availabilities')->whereIn('user_id',$userIds)->whereNull('unavailable_type')->whereNull('deleted_at')->get():[];$ids=array_map(fn($r)=>(int)$r['user_availability_id'],$rows);$texts=[];foreach($ids?DB::table('translations')->where('table_name','user_availabilities')->whereIn('table_id',$ids)->whereIn('field',['summary','description'])->where('locale','fa')->whereNull('deleted_at')->get():[]as$t)$texts[(int)$t['table_id']][$t['field']]=$t['value'];
        $days=['saturday'=>'شنبه','sunday'=>'یکشنبه','monday'=>'دوشنبه','tuesday'=>'سه‌شنبه','wednesday'=>'چهارشنبه','thursday'=>'پنجشنبه','friday'=>'جمعه'];$statuses=['available'=>'فعال','unavailable'=>'غیرفعال','reserved'=>'پر شده','pending'=>'در انتظار تأیید'];$repeats=['week'=>'هفتگی','2-week'=>'دو هفته','3-week'=>'سه هفته','4-week'=>'چهار هفته','month'=>'ماهانه','year'=>'سالانه','none'=>'بی‌تکرار'];$schedules=[];
        $timezoneIds=array_values(array_unique(array_filter(array_map(fn($r)=>(int)($r['timezone_id']??0),$rows))));$timezoneNames=[];foreach($timezoneIds?DB::table('f_timezone')->whereIn('timezone_id',$timezoneIds)->whereNull('deleted_at')->get():[]as$timezoneRow)$timezoneNames[(int)$timezoneRow['timezone_id']]=(string)$timezoneRow['timezone'];foreach($rows as&$scheduleRow)$scheduleRow['timezone']=$timezoneNames[(int)($scheduleRow['timezone_id']??0)]??'Asia/Tehran';unset($scheduleRow);
        foreach($rows as$r){$mid=(int)($r['member_id']??0);$m=$membershipMap[$mid]??null;$start=substr((string)$r['start_time'],0,5);$end=substr((string)$r['end_time'],0,5);$id=(int)$r['user_availability_id'];$slots=[];for($n=((int)substr($start,0,2))*60+(int)substr($start,3,2),$until=((int)substr($end,0,2))*60+(int)substr($end,3,2);$n<$until;$n+=30)$slots[]=sprintf('%02d:%02d',intdiv($n,60),$n%60);$schedules[]=['id'=>$id,'memberId'=>(int)$r['user_id'],'membershipId'=>$mid,'name'=>$names[(int)$r['user_id']]??('کاربر '.$r['user_id']),'role'=>'عضو','day'=>$r['date']?:($days[$r['day_of_week']]??'—'),'timeLabel'=>$start.'-'.$end,'slots'=>$slots,'branchId'=>$m?($m['branch_id']?(int)$m['branch_id']:0):0,'organizationUserId'=>0,'branchName'=>$m?($m['branch_id']?($branchNames[(int)$m['branch_id']]??'شعبه'):($academyNames[(int)$m['academy_id']]??'آموزشگاه')):'سازمان دیگر','status'=>$statuses[$r['status']]??'در انتظار تأیید','repeatPeriod'=>$repeats[$r['repeat_period']]??'هفتگی','repeatDate'=>$r['date']?:'','timezone'=>$r['timezone']??'Asia/Tehran','summary'=>$texts[$id]['summary']??'','description'=>$texts[$id]['description']??'','readOnly'=>!in_array($mid,$manageableIds,true)];}
        return ['members'=>$members,'staff_catalog'=>$staff['staff_catalog']??[],'schedules'=>$schedules,'version'=>sha1(json_encode($schedules,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES))];
    }

    public function saveMemberSchedule(int$actor,array$d,int$id=0,bool$siteAdmin=false):array {
        $catalog=$this->memberSchedules($actor,$siteAdmin);$membershipId=(int)($d['membershipId']??0);$member=null;foreach($catalog['members']as$m)if((int)$m['id']===$membershipId)$member=$m;if(!$member)throw new RuntimeException('عضو انتخاب‌شده معتبر نیست.');
        $days=['شنبه'=>'saturday','یکشنبه'=>'sunday','دوشنبه'=>'monday','سه‌شنبه'=>'tuesday','چهارشنبه'=>'wednesday','پنجشنبه'=>'thursday','جمعه'=>'friday'];$repeats=['هفتگی'=>'week','دو هفته'=>'2-week','سه هفته'=>'3-week','چهار هفته'=>'4-week','ماهانه'=>'month','سالانه'=>'year','بی‌تکرار'=>'none'];$ranges=array_values($d['ranges']??[]);if(!$ranges)throw new RuntimeException('حداقل یک بازه زمانی الزامی است.');$day=$days[$d['day']??'']??null;$date=trim((string)($d['repeatDate']??''));$repeat=$repeats[$d['repeatPeriod']??'هفتگی']??'week';if(!$day&&$date==='')throw new RuntimeException('روز یا تاریخ الزامی است.');$now=date('Y-m-d H:i:s');$pending=$this->staffActorApproves($actor)?false:true;
        $timezoneName=trim((string)($d['timezone']??'Asia/Tehran'))?:'Asia/Tehran';$timezone=DB::table('f_timezone')->where('timezone',$timezoneName)->where('status','active')->whereNull('deleted_at')->first();if(!$timezone)throw new RuntimeException('منطقه زمانی انتخاب‌شده معتبر نیست.');$timezoneId=(int)$timezone['timezone_id'];
        $statusTypes=['فعال'=>'available','غیرفعال'=>'unavailable','پر شده'=>'reserved','در انتظار تأیید'=>'pending','در انتظار تایید'=>'pending'];
        return transaction(function()use($actor,$d,$id,$membershipId,$member,$ranges,$day,$date,$repeat,$now,$pending,$timezoneId,$statusTypes){$saved=[];foreach($ranges as$i=>$range){$start=(string)($range['start']??'');$end=(string)($range['end']??'');if(!preg_match('/^(?:[01]\d|2[0-3]):[0-5]\d$/',$start)||!preg_match('/^(?:[01]\d|2[0-3]):[0-5]\d$/',$end)||$start>=$end)throw new RuntimeException('بازه زمانی معتبر نیست.');$overlap=DB::table('user_availabilities')->where('user_id',(int)$member['user_id'])->whereNull('unavailable_type')->whereNull('deleted_at')->whereRaw('start_time < ? AND end_time > ?',[$end.':00',$start.':00']);if($date!=='')$overlap->where('date',$date);else$overlap->where('day_of_week',$day)->whereNull('date');$existing=$overlap->first();if($existing&&(!$id||(int)$existing['user_availability_id']!==$id))throw new RuntimeException('این عضو در سازمان دیگری در بازه انتخاب‌شده برنامه دارد.');$label=(string)($range['status']??'فعال');if(!isset($statusTypes[$label]))throw new RuntimeException('وضعیت انتخاب‌شده معتبر نیست.');$status=$pending?'pending':$statusTypes[$label];$values=['user_id'=>(int)$member['user_id'],'member_id'=>$membershipId,'date'=>$date?:null,'day_of_week'=>$date?null:$day,'start_time'=>$start.':00','end_time'=>$end.':00','timezone_id'=>$timezoneId,'status'=>$status,'unavailable_type'=>null,'is_repeating'=>$repeat==='none'?0:1,'repeat_period'=>$repeat,'is_closed'=>$status==='unavailable'?1:0,'priority'=>$i+1,'updated_at'=>$now,'updated_by'=>$actor,'approved_at'=>$pending?null:$now,'approved_by'=>$pending?null:$actor,'deleted_at'=>null,'deleted_by'=>null];if($id&&$i===0){DB::table('user_availabilities')->where('user_availability_id',$id)->update($values);$savedId=$id;}else$savedId=DB::table('user_availabilities')->insertGetId(['created_at'=>$now,'created_by'=>$actor]+$values);$this->setTranslations('user_availabilities',$savedId,['summary'=>trim((string)($d['summary']??'')),'description'=>trim((string)($d['description']??''))],$actor);$saved[]=$savedId;}return['ids'=>$saved];});
    }
    public function deleteMemberSchedule(int$actor,int$id,bool$siteAdmin=false):void{$data=$this->memberSchedules($actor,$siteAdmin);$item=null;foreach($data['schedules']as$row)if((int)$row['id']===$id)$item=$row;if(!$item||$item['readOnly'])throw new RuntimeException('اجازه حذف این برنامه زمانی را ندارید.');$now=date('Y-m-d H:i:s');DB::table('user_availabilities')->where('user_availability_id',$id)->update(['deleted_at'=>$now,'deleted_by'=>$actor,'updated_at'=>$now,'updated_by'=>$actor]);}
    public function cycleMemberScheduleStatus(int$actor,int$id,bool$siteAdmin=false):array{$data=$this->memberSchedules($actor,$siteAdmin);$item=null;foreach($data['schedules']as$row)if((int)$row['id']===$id)$item=$row;if(!$item||$item['readOnly'])throw new RuntimeException('اجازه تغییر وضعیت این برنامه زمانی را ندارید.');$row=DB::table('user_availabilities')->where('user_availability_id',$id)->whereNull('unavailable_type')->whereNull('deleted_at')->first();$cycle=['available'=>'unavailable','unavailable'=>'reserved','reserved'=>'pending','pending'=>'available'];$next=$cycle[$row['status']??'pending']??'available';DB::table('user_availabilities')->where('user_availability_id',$id)->update(['status'=>$next,'is_closed'=>$next==='unavailable'?1:0,'approved_at'=>$next==='pending'?null:date('Y-m-d H:i:s'),'approved_by'=>$next==='pending'?null:$actor,'updated_at'=>date('Y-m-d H:i:s'),'updated_by'=>$actor]);return['id'=>$id,'status'=>$next];}

    public function availabilityExceptions(int $actor, bool $siteAdmin=false): array {
        $catalog=$this->memberSchedules($actor,$siteAdmin);$members=$catalog['members'];$organizations=$catalog['staff_catalog']['organizations']??[];$manageableIds=array_map(fn($m)=>(int)$m['id'],$members);$organizationUserIds=array_map(fn($o)=>(int)$o['user_id'],$organizations);$userIds=array_values(array_unique(array_merge(array_map(fn($m)=>(int)$m['user_id'],$members),$organizationUserIds)));
        $membersById=[];$membersByUser=[];foreach($members as$m){$membersById[(int)$m['id']]=$m;$membersByUser[(int)$m['user_id']][]=$m;}
        $organizationsByUser=[];foreach($organizations as$o)$organizationsByUser[(int)$o['user_id']]=$o;
        $allMemberships=$userIds?DB::table('academy_branch_members')->whereIn('user_id',$userIds)->whereNull('deleted_at')->get():[];$allBranchIds=array_values(array_unique(array_filter(array_map(fn($m)=>(int)($m['branch_id']??0),$allMemberships))));$branchMeta=[];foreach($allBranchIds?DB::table('academy_branches')->whereIn('branch_id',$allBranchIds)->whereNull('deleted_at')->get():[]as$b)$branchMeta[(int)$b['branch_id']]=['user_id'=>(int)$b['user_id'],'name'=>$this->tr('academy_branches',(int)$b['branch_id'],'name','شعبه')];foreach($allMemberships as$m)if(!isset($membersById[(int)$m['member_id']])){$base=$membersByUser[(int)$m['user_id']][0]??[];$bid=(int)($m['branch_id']??0);$membersById[(int)$m['member_id']]=$m+['name'=>$base['name']??('کاربر '.$m['user_id']),'organizationUserId'=>$bid?($branchMeta[$bid]['user_id']??0):0,'branchName'=>$bid?($branchMeta[$bid]['name']??'شعبه'):'آموزشگاه'];}
        $rows=$userIds?DB::table('user_availabilities')->whereIn('user_id',$userIds)->whereNotNull('unavailable_type')->whereNull('deleted_at')->get():[];$ids=array_map(fn($r)=>(int)$r['user_availability_id'],$rows);$texts=[];foreach($ids?DB::table('translations')->where('table_name','user_availabilities')->whereIn('table_id',$ids)->whereIn('field',['summary','description'])->where('locale',locale())->whereNull('deleted_at')->get():[]as$t)$texts[(int)$t['table_id']][$t['field']]=$t['value'];
        $timezoneIds=array_values(array_unique(array_filter(array_map(fn($r)=>(int)($r['timezone_id']??0),$rows))));$timezones=[];foreach($timezoneIds?DB::table('f_timezone')->whereIn('timezone_id',$timezoneIds)->whereNull('deleted_at')->get():[]as$t)$timezones[(int)$t['timezone_id']]=$t['timezone'];
        $statusLabels=['available'=>'فعال','unavailable'=>'غیرفعال','pending'=>'در انتظار تأیید'];$typeLabels=['holiday'=>'تعطیل رسمی','closed'=>'تعطیل','unavailable'=>'عدم دسترسی','busy'=>'مشغول','vacation'=>'مرخصی','blocked'=>'مسدود'];$items=[];
        foreach($rows as$r){$id=(int)$r['user_availability_id'];$mid=(int)($r['member_id']??0);$organization=$mid===0?($organizationsByUser[(int)$r['user_id']]??null):null;$m=$organization?:($membersById[$mid]??($membersByUser[(int)$r['user_id']][0]??null));if(!$m)continue;$organizationUserId=$organization?(int)$organization['user_id']:(int)($m['organizationUserId']??0);$branchId=(int)($organization['branch_id']??$m['branch_id']??$m['branchId']??0);$branchName=(string)($organization['name']??$m['branchName']??'سازمان دیگر');$start=$r['start_time']?substr((string)$r['start_time'],0,5):'';$end=$r['end_time']?substr((string)$r['end_time'],0,5):'';$slots=[];if($start&&$end)for($n=((int)substr($start,0,2))*60+(int)substr($start,3,2),$until=((int)substr($end,0,2))*60+(int)substr($end,3,2);$n<$until;$n+=30)$slots[]=sprintf('%02d:%02d',intdiv($n,60),$n%60);$items[]=['id'=>$id,'memberId'=>(int)$r['user_id'],'membershipId'=>$mid,'organizationUserId'=>$organizationUserId,'targetValue'=>$organization?'organization:'.$organizationUserId:'member:'.$mid,'name'=>(string)($organization['name']??$m['name']??('کاربر '.$r['user_id'])),'date'=>(string)$r['date'],'slots'=>$slots,'fullDay'=>!$start||!$end,'timeLabel'=>$start&&$end?$start.'-'.$end:'تمام‌روز','type'=>(string)$r['unavailable_type'],'typeLabel'=>$typeLabels[$r['unavailable_type']]??$r['unavailable_type'],'timezone'=>$timezones[(int)($r['timezone_id']??0)]??'Asia/Tehran','branchId'=>$branchId,'branchName'=>$branchName,'status'=>$statusLabels[$r['status']]??'در انتظار تأیید','summary'=>$texts[$id]['summary']??'','description'=>$texts[$id]['description']??'','readOnly'=>$organization?!in_array((int)$r['user_id'],$organizationUserIds,true):!in_array($mid,$manageableIds,true)];}
        return['members'=>$members,'staff_catalog'=>$catalog['staff_catalog'],'exceptions'=>$items,'version'=>sha1(json_encode($items,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES))];
    }
    public function saveAvailabilityException(int$actor,array$d,int$id=0,bool$siteAdmin=false):array {
        if($id){$editable=null;foreach($this->availabilityExceptions($actor,$siteAdmin)['exceptions']as$item)if((int)$item['id']===$id)$editable=$item;if(!$editable||$editable['readOnly'])throw new RuntimeException('اجازه ویرایش این مورد را ندارید.');}
        $catalog=$this->availabilityExceptions($actor,$siteAdmin);$targetType=($d['targetType']??'member')==='organization'?'organization':'member';$membershipId=(int)($d['membershipId']??0);$member=null;if($targetType==='member')foreach($catalog['members']as$m)if((int)$m['id']===$membershipId)$member=$m;$organization=null;if($targetType==='organization')foreach(($catalog['staff_catalog']['organizations']??[])as$o)if((int)$o['user_id']===(int)($d['organizationUserId']??0))$organization=$o;if(!$member&&!$organization)throw new RuntimeException('شخص یا سازمان انتخاب‌شده معتبر نیست.');$targetUserId=(int)($organization['user_id']??$member['user_id']);$date=trim((string)($d['date']??''));if(!preg_match('/^\d{4}-\d{2}-\d{2}$/',$date)||!strtotime($date))throw new RuntimeException('تاریخ معتبر الزامی است.');$type=(string)($d['type']??'');if(!in_array($type,['holiday','closed','unavailable','busy','vacation','blocked'],true))throw new RuntimeException('نوع عدم دسترسی معتبر نیست.');$allDay=!empty($d['allDay']);$ranges=$allDay?[['start'=>null,'end'=>null]]:array_values((array)($d['ranges']??[]));if(!$ranges)throw new RuntimeException('حداقل یک بازه ساعت تعطیلی الزامی است.');$timezoneName=trim((string)($d['timezone']??'Asia/Tehran'))?:'Asia/Tehran';$timezone=DB::table('f_timezone')->where('timezone',$timezoneName)->whereNull('deleted_at')->first();if(!$timezone)throw new RuntimeException('منطقه زمانی معتبر نیست.');$pending=!$this->staffActorApproves($actor);$now=date('Y-m-d H:i:s');
        return transaction(function()use($actor,$d,$id,$membershipId,$targetUserId,$organization,$date,$type,$ranges,$allDay,$timezone,$pending,$now){$saved=[];foreach($ranges as$i=>$range){$start=$allDay?null:(string)($range['start']??'');$end=$allDay?null:(string)($range['end']??'');if(!$allDay&&(!preg_match('/^(?:[01]\d|2[0-3]):[0-5]\d$/',$start)||!preg_match('/^(?:[01]\d|2[0-3]):[0-5]\d$/',$end)||$start>=$end))throw new RuntimeException('بازه ساعت تعطیلی معتبر نیست.');$values=['user_id'=>$targetUserId,'member_id'=>$organization?null:$membershipId,'date'=>$date,'day_of_week'=>null,'start_time'=>$start===null?null:$start.':00','end_time'=>$end===null?null:$end.':00','timezone_id'=>(int)$timezone['timezone_id'],'status'=>$pending?'pending':'available','unavailable_type'=>$type,'is_repeating'=>0,'repeat_period'=>'none','is_closed'=>$allDay?1:0,'priority'=>$i+1,'approved_at'=>$pending?null:$now,'approved_by'=>$pending?null:$actor,'updated_at'=>$now,'updated_by'=>$actor,'deleted_at'=>null,'deleted_by'=>null];if($id&&$i===0){$existing=DB::table('user_availabilities')->where('user_availability_id',$id)->whereNotNull('unavailable_type')->whereNull('deleted_at')->first();if(!$existing)throw new RuntimeException('رکورد موردنظر یافت نشد.');DB::table('user_availabilities')->where('user_availability_id',$id)->update($values);$savedId=$id;}else$savedId=DB::table('user_availabilities')->insertGetId(['created_at'=>$now,'created_by'=>$actor]+$values);$this->setTranslations('user_availabilities',$savedId,['summary'=>trim((string)($d['summary']??'')),'description'=>trim((string)($d['description']??''))],$actor);$saved[]=$savedId;}return['ids'=>$saved];});
    }
    public function deleteAvailabilityException(int$actor,int$id,bool$siteAdmin=false):void{$data=$this->availabilityExceptions($actor,$siteAdmin);$item=null;foreach($data['exceptions']as$r)if((int)$r['id']===$id)$item=$r;if(!$item||$item['readOnly'])throw new RuntimeException('اجازه حذف این مورد را ندارید.');$now=date('Y-m-d H:i:s');DB::table('user_availabilities')->where('user_availability_id',$id)->whereNotNull('unavailable_type')->update(['deleted_at'=>$now,'deleted_by'=>$actor,'updated_at'=>$now,'updated_by'=>$actor]);}
    public function cycleAvailabilityExceptionStatus(int$actor,int$id,bool$siteAdmin=false):array{$data=$this->availabilityExceptions($actor,$siteAdmin);$item=null;foreach($data['exceptions']as$r)if((int)$r['id']===$id)$item=$r;if(!$item||$item['readOnly'])throw new RuntimeException('اجازه تغییر وضعیت این مورد را ندارید.');$row=DB::table('user_availabilities')->where('user_availability_id',$id)->whereNotNull('unavailable_type')->whereNull('deleted_at')->first();$cycle=['available'=>'unavailable','unavailable'=>'pending','pending'=>'available'];$next=$cycle[$row['status']??'pending']??'available';DB::table('user_availabilities')->where('user_availability_id',$id)->update(['status'=>$next,'approved_at'=>$next==='pending'?null:date('Y-m-d H:i:s'),'approved_by'=>$next==='pending'?null:$actor,'updated_at'=>date('Y-m-d H:i:s'),'updated_by'=>$actor]);return['id'=>$id,'status'=>$next];}
    ...
    private function staffCatalog(int $actor, ?array $academy, bool $siteAdmin): array {
        ...
        elseif($siteAdmin){foreach(DB::table('academies')->whereNull('deleted_at')->get()as$a)$organizations[]=['user_id'=>(int)$a['user_id'],'academy_id'=>(int)$a['academy_id'],'branch_id'=>null,'name'=>$this->tr('academies',(int)$a['academy_id'],'title','آموزشگاه')];foreach(DB::table('academy_branches')->whereNull('deleted_at')->get()as$b)$organizations[]=['user_id'=>(int)$b['user_id'],'academy_id'=>(int)$b['academy_id'],'branch_id'=>(int)$b['branch_id'],'name'=>$this->tr('academy_branches',(int)$b['branch_id'],'name','شعبه')];}
    ...
    private function members(?int $academyId, ?array $branchIds = null): array {
        ...
        $studentData=[];foreach(DB::table('academy_branch_course_term_enrollments')->join('academy_branch_course_terms','academy_branch_course_terms.term_id','=','academy_branch_course_term_enrollments.term_id')->whereIn('academy_branch_course_term_enrollments.member_id',$memberIds)->where('academy_branch_course_term_enrollments.type','student')->whereNull('academy_branch_course_term_enrollments.deleted_at')->orderBy('academy_branch_course_term_enrollments.term_enrollment_id','DESC')->get()as$e){$mid=(int)$e['member_id'];if(isset($studentData[$mid]))continue;$studentData[$mid]=['termId'=>(int)$e['term_id'],'termName'=>$this->tr('academy_branch_course_terms',(int)$e['term_id'],'title','ترم '.(int)$e['term_id'])];}foreach(DB::table('academy_branch_course_term_waiting_list')->join('academy_branch_course_terms','academy_branch_course_terms.term_id','=','academy_branch_course_term_waiting_list.term_id')->whereIn('academy_branch_course_term_waiting_list.member_id',$memberIds)->whereNull('academy_branch_course_term_waiting_list.deleted_at')->orderBy('academy_branch_course_term_waiting_list.term_waiting_list_id','DESC')->get()as$e){$mid=(int)$e['member_id'];if(!isset($studentData[$mid]))$studentData[$mid]=['termId'=>(int)$e['term_id'],'termName'=>$this->tr('academy_branch_course_terms',(int)$e['term_id'],'title','ترم '.(int)$e['term_id'])];}
        $studentLessons=[];foreach(DB::table('user_lessons')->whereIn('user_id',$userIds)->whereNull('deleted_at')->get()as$ul)$studentLessons[(int)$ul['user_id']]=$ul;
        $fatherNames=[];foreach(DB::table('translations')->where('table_name','users')->where('field','father_name')->where('locale','fa')->whereIn('table_id',$userIds)->whereNull('deleted_at')->get()as$t)$fatherNames[(int)$t['table_id']]=$t['value'];
        $addressMap=[];foreach(DB::table('user_addresses')->whereIn('user_id',$userIds)->whereNull('deleted_at')->get()as$a){$province=$a['province_id']?DB::table('world_iran_provinces')->where('province_id',(int)$a['province_id'])->first():null;$county=$a['county_id']?DB::table('world_iran_counties')->where('county_id',(int)$a['county_id'])->first():null;$addressMap[(int)$a['user_id']][]=['province'=>$province['province_name']??'','city'=>$county['county_name']??'','address'=>$this->tr('user_addresses',(int)$a['address_id'],'address',''),'postal_code'=>$a['postal_code'],'lat'=>$a['latitude'],'lng'=>$a['longitude'],'is_main'=>(bool)$a['is_main']];}
        return array_map(function(array $r) use($names,$branches,$lessonNames,$contractTexts,$memberRoles,$academyUsers,$branchUsers,$currencyNames,$studentData,$studentLessons,$fatherNames,$addressMap){
            ...
            return ['id'=>(int)$r['member_id'],'user_id'=>(int)$r['user_id'],'name'=>$names[(int)$r['user_id']]??$r['username'],'phone'=>$r['phone']?:'',
                ...
                'profileVisibility'=>$r['visibility']?:'private','userLessonId'=>(int)($r['user_lesson_id']??0),'lessonId'=>(int)($r['lesson_id']??0),'levelId'=>(int)($r['level_id']??0),'lessons'=>$r['lesson_id']?[['type'=>(int)$r['lesson_id'],'level'=>(int)($r['level_id']??0)]]:[],'lessonName'=>$lessonNames[(int)($r['lesson_id']??0)]??'—','instrument'=>$studentData[(int)$r['member_id']]['termName']??'','termId'=>$studentData[(int)$r['member_id']]['termId']??0,'termKey'=>isset($studentData[(int)$r['member_id']])?(string)$studentData[(int)$r['member_id']]['termId']:'','level'=>'','teacher'=>'','fatherName'=>$fatherNames[(int)$r['user_id']]??'','addresses'=>$addressMap[(int)$r['user_id']]??[],'remaining'=>0,'financial'=>'تسویه','attendance'=>'—','registrationDate'=>$r['joined_at']?:''];
        },$rows);
    }
    ...
    public function updateMember(int $actorId, int $memberId, array $data, bool $siteAdmin = false): array {
        return transaction(function() use($actorId,$memberId,$data,$siteAdmin){
            ...
            $organization['kind']=$organization['branch_id']===null?'academy':'branch';$organization['id']=(int)($organization['branch_id']??$organization['academy_id']);
            if(str_contains((string)$member['user_id'],self::STUDENT_USERNAME_MARKER)||($data['type']??'')==='student'){$termId=(int)($data['termId']??0);$term=DB::table('academy_branch_course_terms')->join('academy_branch_courses','academy_branch_courses.course_id','=','academy_branch_course_terms.course_id')->where('academy_branch_course_terms.term_id',$termId)->whereIn('academy_branch_course_terms.status',['open','ongoing'])->whereNull('academy_branch_course_terms.deleted_at')->whereNull('academy_branch_courses.deleted_at')->first();if(!$term)throw new RuntimeException('ترم آموزشی معتبر نیست.');$belongs=$organization['kind']==='academy'?(int)$term['academy_id']===(int)$organization['id']:(int)$term['branch_id']===(int)$organization['id'];if(!$belongs)throw new RuntimeException('ترم آموزشی متعلق به سازمان انتخاب‌شده نیست.');DB::table('academy_branch_members')->where('member_id',$memberId)->update(['academy_id'=>(int)$organization['academy_id'],'branch_id'=>$organization['branch_id'],'updated_at'=>$now,'updated_by'=>$actorId]+$approval);$enrollment=DB::table('academy_branch_course_term_enrollments')->where('term_id',$termId)->where('member_id',$memberId)->where('type','student')->first();$values=['status'=>'active','joined_at'=>$data['registrationDate']??date('Y-m-d'),'updated_by'=>$actorId,'deleted_at'=>null,'deleted_by'=>null]+$approval;if($enrollment)DB::table('academy_branch_course_term_enrollments')->where('term_enrollment_id',(int)$enrollment['term_enrollment_id'])->update($values);else DB::table('academy_branch_course_term_enrollments')->insert(['term_id'=>$termId,'member_id'=>$memberId,'type'=>'student','created_by'=>$actorId]+$values);return['member_id'=>$memberId];}
    ...
    public function store(int $ownerUserId, array $data, bool $siteAdmin = false): array {
        return transaction(function () use ($ownerUserId, $data, $siteAdmin) {
            ...
            $branchUserId = DB::table('users')->insertGetId([
                ...
                'type' => 'branch', 'status' => $this->userStatus($data['status'] ?? null),
    ...
    public function update(int $ownerUserId, int $branchId, array $data, bool $siteAdmin = false): array {
        return transaction(function () use ($ownerUserId, $branchId, $data, $siteAdmin) {
            ...
            DB::table('users')->where('user_id', (int)$branch['user_id'])->update([
                'username' => $data['username'], 'status' => $this->userStatus($data['status'] ?? null), 'updated_by' => $ownerUserId,
            ]);
    ...
    public function cycleStatus(int $ownerUserId, int $branchId, bool $siteAdmin = false): array {
        if (!$siteAdmin && !$this->canAccessBranch($ownerUserId, $branchId)) throw new RuntimeException('دسترسی تغییر وضعیت این شعبه را ندارید.');
        $branch = DB::table('academy_branches')->where('branch_id', $branchId)->whereNull('deleted_at')->first();
        if (!$branch) throw new RuntimeException('شعبه یافت نشد.');
        $user = DB::table('users')->where('user_id', (int)$branch['user_id'])->whereNull('deleted_at')->first();
        if (!$user) throw new RuntimeException('حساب کاربری شعبه یافت نشد.');
        $next = match ((string)($user['status'] ?? 'inactive')) {
            'approved' => 'inactive',
            'inactive' => 'removed',
            default => 'approved',
        };
        DB::table('users')->where('user_id', (int)$branch['user_id'])->update(['status'=>$next, 'updated_by'=>$ownerUserId]);
        return $this->decorate($branch);
    }
    ...
    private function userStatus(mixed $status): string {
        return match (trim((string)$status)) {
            'active', 'approved', 'فعال' => 'approved',
