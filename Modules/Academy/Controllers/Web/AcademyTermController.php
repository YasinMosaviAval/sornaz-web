...
class AcademyTermController
{
    ...
    public function index(){return $this->run(fn()=>['success'=>true,'data'=>$this->service->bootstrap((int)auth()->id())]);}
    public function invoices(){return $this->run(fn()=>['success'=>true,'data'=>$this->service->invoiceBootstrap((int)auth()->id())]);}
    public function updateInvoice(int$id){return $this->run(function()use($id){$this->service->updateInvoice((int)auth()->id(),$id,$this->payload());return['success'=>true];});}
    public function payInstallment(int$id,int$installmentId){return $this->run(function()use($id,$installmentId){$this->service->payInstallment((int)auth()->id(),$id,$installmentId);return['success'=>true];});}
    public function store(){return $this->save();}
    public function update(int$id){return $this->save($id);}
    public function storeDiscount(){return $this->run(fn()=>['success'=>true,'data'=>$this->service->saveDiscount((int)auth()->id(),$this->payload())]);}
    public function destroy(int$id){return $this->run(function()use($id){$this->service->delete((int)auth()->id(),$id);return['success'=>true];});}
    ...
    {
        try{return ResponseFactory::json($callback());}
