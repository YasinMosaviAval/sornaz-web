...
<style>
    ...
    .comment-editor a { color: #4f46e5; text-decoration: underline; }
...
<div id="page-article-detail">
    <div class="max-w-6xl mx-auto px-4 py-10 md:py-14">
        ...
        <div class="mt-10 rounded-2xl border border-amber-100 bg-amber-50/60 p-4"><div class="mb-2 text-sm font-medium text-gray-700"><?= $isEnglish?'Rate this article':'امتیاز به این مقاله' ?></div><div data-public-rating data-rating-type="post" data-rating-id="<?= (int)$article['id'] ?>"></div></div>

        <section class="mt-14 border-t border-gray-100 pt-10" id="articleComments" dir="<?= $isEnglish ? 'ltr' : 'rtl' ?>">
            <h2 class="text-2xl font-bold mb-6"><?= $isEnglish ? 'User comments' : 'نظرات کاربران' ?></h2>
            <div class="space-y-5 mb-10">
                <?php if (empty($comments)): ?><p class="rounded-2xl bg-gray-50 p-5 text-gray-500"><?= $isEnglish ? 'No comments have been posted for this article yet.' : 'هنوز نظری برای این مقاله ثبت نشده است.' ?></p><?php endif; ?>
                <?php foreach (($comments ?? []) as $comment): $depth=(int)($comment['depth']??0); ?><article class="rounded-2xl border border-gray-100 bg-white p-5" style="margin-inline-start:<?= min($depth,3)*2 ?>rem"><div class="flex items-center justify-between gap-3 mb-3"><strong><?= htmlspecialchars($comment['author']) ?></strong><time class="text-xs text-gray-400"><?= htmlspecialchars((string)$comment['created_at']) ?></time></div><div class="text-gray-600 leading-8 comment-html"><?= $comment['content'] ?></div><div class="mt-4 flex flex-wrap items-end justify-between gap-3"><div data-public-rating data-rating-type="comment" data-rating-id="<?= (int)$comment['id'] ?>"></div><button type="button" class="comment-reply-button text-sm font-medium text-indigo-600" data-comment-id="<?= (int)$comment['id'] ?>" data-comment-author="<?= htmlspecialchars($comment['author'],ENT_QUOTES) ?>"><i class="fas fa-reply ml-1"></i><?= $isEnglish?'Reply':'پاسخ' ?></button></div></article><?php endforeach; ?>
            </div>
            <div class="rounded-3xl bg-gray-50 p-6 md:p-8">
                <h3 id="commentFormTitle" class="text-xl font-bold mb-2"><?= $isEnglish ? 'Submit a new comment' : 'ارسال نظر جدید' ?></h3><button type="button" id="cancelCommentReply" class="mb-4 hidden text-sm text-gray-500 hover:text-red-600"><?= $isEnglish?'Cancel reply':'لغو پاسخ' ?></button>
                <form id="publicCommentForm" class="space-y-4"><input type="hidden" name="_token" value="<?= e(csrf_token()) ?>"><?php $viewer=auth()->user(); if(!$viewer): ?><div class="grid md:grid-cols-2 gap-4"><label class="block text-sm"><?= $isEnglish ? 'Full name' : 'نام و نام خانوادگی' ?><input name="author" placeholder="<?= $isEnglish ? 'Full name' : 'نام و نام خانوادگی' ?>" class="mt-2 w-full rounded-2xl border border-gray-200 bg-white px-4 py-3"></label><label class="block text-sm"><?= $isEnglish ? 'Email' : 'ایمیل' ?><input name="author_email" type="email" placeholder="<?= $isEnglish ? 'Email' : 'ایمیل' ?>" class="mt-2 w-full rounded-2xl border border-gray-200 bg-white px-4 py-3"></label></div><?php endif; ?><div class="block text-sm"><?= $isEnglish ? 'Your comment' : 'نظر شما' ?><div class="mt-2 overflow-hidden rounded-2xl border border-gray-200 bg-white"><div class="flex gap-2 border-b border-gray-100 p-2" dir="<?= $isEnglish ? 'ltr' : 'rtl' ?>"><input type="url" id="commentLinkUrl" placeholder="<?= $isEnglish ? 'Link URL' : 'آدرس لینک' ?>" class="w-full max-w-xs rounded-lg border border-gray-200 px-3 py-1 text-sm outline-none"><button type="button" id="commentLinkButton" class="rounded-lg px-3 py-1 text-indigo-600 hover:bg-gray-100" title="<?= $isEnglish ? 'Insert link' : 'افزودن لینک' ?>">🔗</button></div><div id="commentEditor" contenteditable="true" data-placeholder="<?= $isEnglish ? 'Write your comment ...' : 'نظر خود را بنویسید ...' ?>" class="comment-editor min-h-[140px] px-4 py-3 outline-none"></div><textarea name="content" id="commentContent" class="hidden"></textarea></div></div><div class="flex justify-end"><button type="submit" class="rounded-2xl bg-indigo-600 px-6 py-3 font-medium text-white hover:bg-indigo-700"><?= $isEnglish ? 'Submit comment' : 'ارسال نظر' ?></button></div><p id="commentFormMessage" class="text-sm"></p></form>
            </div>
        </section>
...
<script>
const commentEditor=document.getElementById('commentEditor'),commentContent=document.getElementById('commentContent');
const commentForm=document.getElementById('publicCommentForm'),commentParent=document.createElement('input');commentParent.type='hidden';commentParent.name='parent';commentParent.value='0';commentForm?.appendChild(commentParent);const resetReply=()=>{commentParent.value='0';document.getElementById('commentFormTitle').textContent=<?= $isEnglish?"'Submit a new comment'":"'ارسال نظر جدید'" ?>;document.getElementById('cancelCommentReply').classList.add('hidden')};document.querySelectorAll('.comment-reply-button').forEach(button=>button.addEventListener('click',()=>{commentParent.value=button.dataset.commentId;document.getElementById('commentFormTitle').textContent=<?= $isEnglish?"'Reply to ' + button.dataset.commentAuthor":"'پاسخ به ' + button.dataset.commentAuthor" ?>;document.getElementById('cancelCommentReply').classList.remove('hidden');commentForm.scrollIntoView({behavior:'smooth',block:'center'});commentEditor.focus()}));document.getElementById('cancelCommentReply')?.addEventListener('click',resetReply);
commentForm?.addEventListener('reset',()=>setTimeout(resetReply,0));
document.getElementById('commentLinkButton')?.addEventListener('click',function(event){event.preventDefault();event.stopPropagation();const url=document.getElementById('commentLinkUrl')?.value.trim();if(!url)return;commentEditor.focus();const selected=window.getSelection()?.toString();if(selected)document.execCommand('createLink',false,url);else document.execCommand('insertHTML',false,'<a href="'+url+'" target="_blank" rel="noopener">'+url+'</a>');});
document.getElementById('publicCommentForm')?.addEventListener('submit',async function(e){e.preventDefault();const m=document.getElementById('commentFormMessage'),b=this.querySelector('button[type="submit"]');commentContent.value=commentEditor.innerHTML.trim();if(!commentEditor.textContent.trim()){m.textContent=<?= $isEnglish ? "'Comment text is required.'" : "'متن نظر الزامی است.'" ?>;m.className='text-sm text-red-600';return;}b.disabled=true;try{const data=new FormData(this),r=await fetch('/analytics/article-comments/<?= (int)($article['id']??0) ?>',{method:'POST',headers:{Accept:'application/json','X-Requested-With':'XMLHttpRequest','X-CSRF-TOKEN':data.get('_token')||''},body:data}),payload=await r.json(),j=payload.data||payload;if(!r.ok||!j.success)throw new Error(j.message||<?= $isEnglish ? "'Comment submission failed.'" : "'ارسال نظر ناموفق بود.'" ?>);m.textContent=j.message;m.className='text-sm text-emerald-600';this.reset();commentEditor.innerHTML='';commentContent.value='';}catch(x){m.textContent=x.message;m.className='text-sm text-red-600';}finally{b.disabled=false;}});
</script>
