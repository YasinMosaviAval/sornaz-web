...
class AdminTestController {
    ...
    public function runAllTests() {
        ...
        try{$actor=(int)auth()->id();$report=[];
            ...
            $a=$this->tests->seedAcademyManagers((int)($_POST['manager_count']??10),$managerOptions);$report[]=['title'=>'مدیران','items'=>["ایجاد: {$a['created']}","همگام‌سازی: {$a['updated']}","مجموع هدف: {$a['total']}"]];
            $b=$this->academies->seedSamples((int)($_POST['academy_count']??10));$report[]=['title'=>'آموزشگاه‌ها و شعب اصلی','items'=>["ایجاد: {$b['created']}","همگام‌سازی: {$b['updated']}","شعب اصلی پردازش‌شده: {$b['branches_created']}"]];
