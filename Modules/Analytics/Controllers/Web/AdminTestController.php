...
class AdminTestController {
    ...
    public function seedBranchNetworkCoursesAndTerms(){
        if(env('APP_ENV','production')!=='local')abort(404);
        try{
            $actor=(int)auth()->id();
            $options=[];
            foreach(['branches','teachers','receptionists','employees','managers','students'] as $key){
                $options[$key.'_min']=(int)($_POST[$key.'_min']??0);
