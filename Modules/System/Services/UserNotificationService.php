...
class UserNotificationService {
    public function send(int $userId, string $title, string $message, string $entityType, int $entityId, ?int $createdBy = null, ?string $englishTitle = null, ?string $englishMessage = null): void {
        ...
        $id = (int)DB::table('user_messages')->insertGetId(['sender_id'=>$createdBy,'receiver_user_id'=>$userId,'type'=>'notification','status'=>'published','related_entity_type'=>$entityType,'related_entity_id'=>$entityId,'is_read'=>0,'created_by'=>$actor,'updated_by'=>$actor]);
        foreach (['fa'=>[$title,$message],'en'=>[$englishTitle ?: $this->englishTitle($title),$englishMessage ?: $this->englishMessage($message)]] as $locale => [$translatedTitle,$translatedMessage]) {
            foreach (['title'=>$translatedTitle,'message'=>$translatedMessage] as $field=>$value) DB::table('translations')->insert(['table_name'=>'user_messages','table_id'=>$id,'field'=>$field,'locale'=>$locale,'value'=>$value,'version'=>1,'created_by'=>$actor,'updated_by'=>$actor]);
        }
    }
