...
<div class="wrapper">
    ...
    <div class="content">
        <div class="header_ac"><h1 class="h1_ac">گزینه های نظرسنجی</h1></div>
        ...
        <table>
            ...
            <tbody>
                <? foreach($poll_options as $key => $poll_option) { ?>
                    <tr>
                        <!-- <td><?//= $poll_option['user_poll_option_id']?></td> -->
                        <td><?= $key + 1 ?></td>
                        <td><?= $poll_option['poll_id']?></td>
                        <td><?= $poll_option['title']?></td>
                        <td><?= $poll_option['votes_count']?></td>
                        <td><?= $poll_option['sort_order']?></td>
                        <td><?= $poll_option['created_at']?></td>
                        <td><?= $users[$poll_option['created_by']]['title'] ?></td>
                        <td><?= $poll_option['updated_at'] ?></td>
                        <td><?= $users[$poll_option['updated_by']]['title'] ?></td>
                        <td><?= $poll_option['approved_at'] ?></td>
                        <td><?= $users[$poll_option['approved_by']]['title'] ?? '' ?></td>
                    </tr>
                <? } ?>
            </tbody>
        </table>
...
<br>
        ...
        <form method="POST" action="<?=baseUrl()?>/admin/add_new_user_poll_option/">
            ...
            <input type="hidden" name="votes_count" id="votes_count" value="<?= $poll['votes_count'] ?>" />
            ...
            <br>
            <button type="submit">ثبت گزینه نظرسنجی</button>
