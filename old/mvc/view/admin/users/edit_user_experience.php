...
<div class="wrapper">
    ...
    <div class="content">
        <div class="header_ac">
            <h1 class="h1_ac">تجربه ها</h1>
            <p>تجربه هایی که هنوز به پایان نرسیده اند با رنگ سبز در جدول مشخص شده اند!</p>
        </div>
        ...
        <div class="comments-filter-bar">
            <ul class="filter-list">
                ...
                <li class="filter-item active"><a href="<?=baseUrl() . $settings['user_panel_topbar_1_sidebar_1']['url'] ?>"><?= translate($settings, 'user_panel_topbar_1_sidebar_1') ?> <span class="count">(<?//= sizeof($branches_members)?>)</span></a></li>
...
<br>
        ...
        <? foreach($branches as $branch_id => $branch) { ?>
            ...
            <table>
                ...
                <tbody>
                    <? foreach($experiences[$branch_id] as $key => $experience) { ?>
                        <tr style="<?= $experience['end_date'] == null ? 'background-color: #00ff00;' : '' ?>">
                            <!-- <td><?//= $experience['user_experience_id'] ?></td> -->
                            <!-- <td><?//= $experience['user_id'] ?></td> -->
                            <td><?= $key + 1 ?></td>
                            <!-- <td><?//= $experience['address_id'] ?></td> -->
                            <td><?= $experience['text_1'] ?></td>
                            <td><?= $experience['title'] ?></td>
                            <td><?= $experience['start_date'] ?></td>
                            <td><?= $experience['end_date'] ?></td>
                            <td><?= $experience['created_at'] ?></td>
                            <td><?= $users[$experience['created_by']]['title'] ?></td>
                            <td><?= $experience['updated_at'] ?></td>
                            <td><?= $users[$experience['updated_by']]['title'] ?></td>
                            <? if($experience['approved_by'] === null) { ?>
                                <td colspan="2" style="text-align: center;">
                                    <a href="<?=baseUrl()?>/admin/approved_user_experience/<?= $experience['user_experience_id'] ?>">
                                        <button>تایید تجربه</button>
                                    </a>
                                </td>
                            <? } else { ?>
                                <td><?= $experience['approved_at'] ?></td>
                                <td><?= $users[$experience['approved_by']]['title'] ?? '' ?></td>
                            <? } ?>
                        </tr>
                        <?//= dump($experience) ?>
                    <? } ?>
...
<br>
        ...
        <form method="POST" action="<?=baseUrl()?>/admin/add_new_user_experience/">
            ...
            <input type="hidden" name="subject_1" value="company" />
            ...
            <div><label for="text_1">شرکت</label><input type="text" id="text_1" name="text_1" required></div>
            ...
            <br>
            <button type="submit">ثبت تجربه</button>
