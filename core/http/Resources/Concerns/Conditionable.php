...
trait Conditionable {
    ...
    protected function whenNull(mixed $value, mixed $result): mixed {
        return is_null($value) ? value($result) : MissingValue::make();
    }


    protected function whenNotNull(mixed $value): mixed {
        return !is_null($value) ? $value : MissingValue::make();
    }


    protected function whenHas(mixed $value, mixed $result = null): mixed {
        if ($value !== null && $value !== '' && $value !== []) {
            return $result === null ? $value : value($result);
        }
        return MissingValue::make();
    }
    ...
    protected function mergeWhen(bool $condition, array $attributes): mixed {
        if ($condition) {
            return new MergeValue($attributes);
        }
        return MissingValue::make();
    ...
    protected function mergeUnless(bool $condition, array $attributes): mixed {
